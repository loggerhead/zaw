...
pub fn build(b: *std.Build) !void {
    ...
    _ = b.addModule("zaw", .{
        .root_source_file = b.path("implementations/wasm-zig/src/zaw.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const conduit_tests = b.addTest(.{ .root_module = b.createModule(.{
        .root_source_file = b.path("implementations/wasm-zig/src/conduit/conduit.test.zig"),
        .target = target,
    }) });
    const interop_tests = b.addTest(.{ .root_module = b.createModule(.{
        .root_source_file = b.path("implementations/wasm-zig/src/interop.test.zig"),
        .target = target,
    }) });
